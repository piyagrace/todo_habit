...
const ProfileScreen = () => {
  ...
  const fetchProfileData = async () => {
    try {
      ...
      const userResponse = await axios.get(`http://10.0.2.2:3001/users/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setUserName(userResponse.data.user.name);

      const todosCountResponse = await axios.get(`http://10.0.2.2:3001/users/${userId}/todos/count`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const fetchWeeklyStats = async () => {
    try {
      const userId = await AsyncStorage.getItem("userId");
      if (!userId) return;
      const response = await axios.get(`http://10.0.2.2:3001/users/${userId}/todos/weekly-stats`);
