...
const ProfileScreen = () => {
  ...
  const fetchProfileData = async () => {
    try {
      ...
      const todosCountResponse = await axios.get(`http://localhost:3001/users/${userId}/todos/count`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const fetchWeeklyStats = async () => {
    try {
      const userId = await AsyncStorage.getItem("userId");
      if (!userId) return;
      const response = await axios.get(`http://localhost:3001/users/${userId}/todos/weekly-stats`);
